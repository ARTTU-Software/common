...
1. **Generated definitions** — structs and macros auto-generated from the DBC file. These define what each CAN message looks like (fields, IDs, scaling factors).
2. **Generic CAN driver** — a hardware-agnostic driver that handles TX scheduling, TX queuing, and RX buffering via ring buffers. It talks to the HAL through function pointers you supply at init.

Each board project then provides two files that wire these together:
...
> [!TIP]
> See the [CAN Integration Guide](/shared/common/modules/drivers/can-integration) for a step-by-step walkthrough of setting this up on a new board, with annotated examples from the ECU.
...
> [!WARNING]
> Do not edit the generated headers by hand. Re-run the generator instead.
...
HAL RX interrupt                     CAN_send_frames()
  └─► CAN_driver_rx_callback()         └─► enqueues due periodic frames
        └─► writes to RX ring buf          into the TX ring buffer
                                     ...
                                     process_can_frames()  [your code]
                                       └─► reads from RX ring buffer
                                            via head/tail iteration
```

**TX scheduling**: Each TX frame slot has a `scheduler_timer_value` (period in ms). `CAN_send_frames()` checks which frames are due and enqueues them. Frames marked `CAN_DRIVER_NON_PERIODIC_FRAME` are skipped by the scheduler — you send them manually via `CAN_send_single_frame()`.

**TX deduplication**: If a frame with the same CAN ID is already queued, the driver replaces it instead of appending. This ensures the bus always carries the freshest data.

**RX flow**: The ISR callback writes incoming frames into the RX ring buffer. Your `process_can_frames()` function in the main loop reads them out by iterating from `tail` to `head`.

> [!NOTE]
> The driver uses `void*` for the HAL handle and TX headers to stay hardware-agnostic. You cast to your platform's types (e.g. `FDCAN_HandleTypeDef*`) in `can_driver.c`.

### Key types

| Type | Purpose |
|------|---------|
| `CAN_Driver_t` | Main driver instance — holds buffer pointers, frame configs, HAL function pointers |
| `CAN_Tx_Message_Frame_t` | TX frame slot — ID, payload, scheduler timer, HAL header pointer |
| `CAN_Rx_Message_Frame_t` | RX frame — ID, payload, timestamp |
...
CAN_DRIVER_NON_PERIODIC_FRAME // Sentinel: scheduler skips this frame
```
...
Initializes the driver. Call this **after** you've set `tx_message_frames`, `rx_ring_buffer.frame`, `tx_ring_buffer.frame`, frame counts, and buffer sizes on the driver struct.

| Parameter | Description |
|-----------|-------------|
| `driver` | `CAN_Driver_t*` — driver instance to initialize |
| `add_to_fifo_fn` | `CanTxFn_t` — HAL function to push a frame into the hardware FIFO (e.g. `HAL_FDCAN_AddMessageToTxFifoQ`) |
| `get_tx_fifo_level_fn` | `CanTxFifoLevelFn_t` — HAL function to check free slots in the hardware TX FIFO |
| `hfdcan_instance` | `void*` — hardware peripheral handle |

#### `void CAN_driver_rx_callback(driver, data, hdr_rx, msg_id, num_values, timestamp)`

ISR-safe callback. Copies the incoming frame into the RX ring buffer and sets `can_new_message_flag`. Call this from your `HAL_FDCAN_RxFifo0Callback`.

> [!WARNING]
> This runs in interrupt context. Do not do heavy processing here.

| Parameter | Description |
|-----------|-------------|
| `data` | `uint8_t*` — received payload bytes |
| `hdr_rx` | `void*` — RX header (platform-specific) |
| `msg_id` | Message identifier |
| `num_values` | Payload length |
| `timestamp` | `HAL_GetTick()` or equivalent |

#### `void CAN_send_frames(driver, current_tick)`

Iterates all TX frame slots and enqueues any whose scheduler timer has elapsed. Call this every main loop iteration.

The scheduler rotates its start index each call to prevent fixed-priority bias (i.e. earlier frame slots don't always get serviced first).

#### `uint16_t CAN_process_tx_queue(driver, amount)`

Drains queued TX frames from the software ring buffer into the hardware FIFO. Stops when the FIFO is full or `amount` frames have been sent (0 = drain all).

Returns the number of frames actually moved into the FIFO.

#### `uint32_t CAN_send_single_frame(driver, frame)`

Enqueues a single frame for immediate transmission. Use this for non-periodic frames (status messages, event-driven sends).

Returns `0` on success, `1` if the TX queue is full.

```c
// Pattern: try to send, flush if queue was full, retry
...
<details>
<summary>Click to expand full state and flow diagram</summary>
